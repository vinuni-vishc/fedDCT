...
class ResNet(nn.Module):
	...
	def _forward_impl(self, x):
		...
		#x = self.bn1(x)
		#x = self.relu(x)
		#x = self.maxpool(x) #128,3,32,32 #128,3,64,64 
		#print(self.layer0)
		#print(x.shape)
		x = self.layer0(x) #128,16,32,32 #128,128,16,16 

		x = self.layer1(x) #128,128,32,32 #128,256,16,16 
		x = self.layer2(x) #128,256,32,32 #128,512,8,8 
		x = self.layer3(x) #128,512,8,8 #128,1024,4,4 
		if self.layer4 is not None:
			x = self.layer4(x) #128,2048,2,2 

		x = self.avgpool(x) #128,512,1,1 #128,2048,1,1
		x = torch.flatten(x, 1) #128,512 #128,2048
		
		#x = x.view(x.size(0), -1)
		if self.dropout is not None:
			x = self.dropout(x)
		
		x = self.fc(x) #128,100 #128,7
