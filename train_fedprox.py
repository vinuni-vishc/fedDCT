...
def main(args):
    ...
    if args.multiprocessing_distributed:
        ...
        print("INFO:PyTorch: The number of GPUs in this node is {}".format(
            ngpus_per_node))
...
def client_update(args, current_round, client_model,global_model, scheduler, optimizer, train_loader, epoch=5, streams=None, scaler=None):
    client_model.train()
    # loop local epoch
    ...
    # compute gradient and do SGD step
            ...
            total_loss = total_loss + + (args.mu / 2) * proximal_term
            if args.is_amp:
...
def main_worker(gpu, ngpus_per_node, args):
    ...
    setproctitle.setproctitle(
        args.proc_name + 'fedavg_rank{}'.format(args.rank))
    ...
    print("INFO:PyTorch: using FedProx optimizer.")
    ...
    for r in range(args.start_round, args.num_rounds + 1):
        ...
        if (args.fixed_cluster == 1):
            ...
            for i in tqdm(range(args.num_selected)):
                ...
                current_client_idx = np.arange(start=current_cluster_idx*args.split_factor, stop=(
                    current_cluster_idx+1)*(args.split_factor), step=1)
                ...
                for j in tqdm(range(args.split_factor)):
                    ...
                    loss += client_update(args, r, client_models[i],global_model, schedulers[i], optimizers[i],
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='FedProx Training')
