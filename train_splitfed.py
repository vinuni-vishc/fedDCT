...
def server_aggregate(global_model_client,global_model_server,models_client, models_server):
    """
    This function has aggregation method 'mean'
    """
    ### This will take simple mean of the weights of models ###
    global_client_dict = global_model_client.state_dict()
    global_model_server_dict = global_model_server.state_dict()

    for k in global_client_dict.keys():
        global_client_dict[k] = torch.stack([models_client[i].state_dict(
        )[k].float() for i in range(len(models_client))], 0).mean(0)
    global_model_client.load_state_dict(global_client_dict)

    for k in global_model_server_dict.keys():
        global_model_server_dict[k] = torch.stack([models_server[i].state_dict(
        )[k].float() for i in range(len(models_server))], 0).mean(0)
    global_model_server.load_state_dict(global_model_server_dict)
    
    for model in models_client:
        model.load_state_dict(global_model_client.state_dict())

    for model in models_server:
        model.load_state_dict(global_model_server.state_dict())

def client_update(args, current_round, model_client,model_server, scheduler_client,scheduler_server, optimizer_client,optimizer_server, train_loader, epoch=5, streams=None):
    model_client.train()
    model_server.train()
    for e in range(epoch):
        ...
        while images is not None:
            # adjust the lr first'
            scheduler_client(optimizer_client, i, current_round)
            scheduler_server(optimizer_server, i, current_round)
            i += 1
            fx,y_a, y_b, lam = model_client(images,
                                    target=target,
                                    mode='train',
                                    epoch=epoch,
                                    streams=streams)
            ...
            ensemble_output, outputs, ce_loss, cot_loss = model_server(main_client_fx,y_a, y_b, lam,target=target,
                                    mode='train',
                                    epoch=epoch,
                                    streams=streams)
...
def validate(val_loader, model_client,model_server, args, streams=None):
    ...
    with torch.no_grad():
        ...
        for i, (images, target) in enumerate(val_loader):
            ...
            for j in range(args.loop_factor):
                main_client_fx.append(fx[j].clone().detach().requires_grad_(True))
            ensemble_output, outputs, ce_loss = model_server(main_client_fx,target=target,
                                    mode='val',
                                    )
...
def main_worker(gpu, ngpus_per_node, args):
    ...
    if args.gpu is not None:
        if not args.evaluate:
            ...
            print("INFO:PyTorch: Use GPU: {} for evaluating, the rank of this GPU is {}".format(
                args.gpu, args.rank))
    setproctitle.setproctitle(args.proc_name + 'splitfed_rank{}'.format(args.rank))
    ...
    if args.is_summary:
        ...
        return None
    summary.save_model_to_json(args, global_model_client)
    summary.save_model_to_json(args, global_model_server)
    ...
    param_groups_clients_models = [model.parameters(
    ) if args.is_wd_all else lr_scheduler.get_parameter_groups(model) for model in models_client]

    param_groups_servers_models = [model.parameters(
    ) if args.is_wd_all else lr_scheduler.get_parameter_groups(model) for model in models_server]
    ...
    if args.optimizer == 'SGD':
        ...
        optimizers_clients_models = [torch.optim.SGD(param_group,
                                      args.lr,
                                      momentum=args.momentum,
                                      weight_decay=args.weight_decay,
                                      nesterov=True if args.is_nesterov else False
                                      ) for param_group in param_groups_clients_models]
        optimizers_servers_models = [torch.optim.SGD(param_group,
                                      args.lr,
                                      momentum=args.momentum,
                                      weight_decay=args.weight_decay,
                                      nesterov=True if args.is_nesterov else False
                                      ) for param_group in param_groups_servers_models]
    else:
    ...
    if args.resume:
        resume_proxy = (args.resume).replace("client","server")
        if os.path.isfile(args.resume) and os.path.isfile(resume_proxy):
            print("INFO:PyTorch: => loading checkpoint '{}'".format(args.resume))
            if args.gpu is None:
                checkpoint_model_client = torch.load(args.resume)
                checkpoint_model_server = torch.load(resume_proxy)
            else:
                # Map model to be loaded to specified single gpu.
                loc = 'cuda:{}'.format(args.gpu)
                checkpoint_model_client = torch.load(args.resume, map_location=loc)
                checkpoint_model_server = torch.load(resume_proxy, map_location=loc)
            args.start_round = checkpoint_model_client['round']
            best_acc1 = checkpoint_model_server['best_acc1']
            print("INFO:PyTorch: Loading state_dict of main client")
            global_model_client.load_state_dict(checkpoint_model_client['state_dict'])
            for optimizer in optimizers_clients_models:
                optimizer.load_state_dict(checkpoint_model_client['optimizer'])
            print("INFO:PyTorch: Loading state_dict of proxy clients")
            global_model_server.load_state_dict(checkpoint_model_server['state_dict'])
            optimizers_servers_models.load_state_dict(checkpoint_model_server['optimizer'])
            print("INFO:PyTorch: => loaded checkpoint '{}' (epoch {})".format(
                args.resume, checkpoint_model_client['round']))
            print("INFO:PyTorch: => loaded checkpoint '{}' (epoch {})".format(
                resume_proxy, checkpoint_model_server['round']))
    ...
    schedulers_models_clients = [lr_scheduler.lr_scheduler(mode=args.lr_mode,
                                          init_lr=args.lr,
    ...
    schedulers_models_server = [lr_scheduler.lr_scheduler(mode=args.lr_mode,
                                          init_lr=args.lr,
                                          num_epochs=args.num_rounds,
                                          iters_per_epoch=len(train_loader),
                                          lr_milestones=args.lr_milestones,
                                          ...
                                          decay_epochs=args.decay_epochs,
                                          staircase=True
                                          )for optimizer in optimizers_servers_models]
    ...
    for r in range(args.start_round, args.num_rounds + 1):
        ...
        for i in tqdm(range(args.num_selected)): # number of clusters chosen perround
            ...
            # client_update(args,client_model,scheduler,optimizer,train_loader,epoch=5,streams=None,scaler=None)
                ...
                loss += client_update(args,r, models_client[i],models_server[i], schedulers_models_clients[i],schedulers_models_server[i],optimizers_clients_models[i],optimizers_servers_models[i],
                                    train_loader[current_client_idx[j]], epoch=args.fed_epochs, streams=streams) # train model with data from each client
                val_writer.add_scalar(
                    'learning_rate_'+str(i), optimizers_clients_models[i].param_groups[0]['lr'], global_step=r)
                val_writer.add_scalar(
                    'learning_rate_proxy'+str(i), optimizers_servers_models[i].param_groups[0]['lr'], global_step=r)
        ...
        server_aggregate(global_model_client,global_model_server,models_client, models_server)
        test_loss, acc,top1_all = validate(val_loader, global_model_client,global_model_server, args)
        ...
        # for sl in range(args.num_selected+1):
        ...
        ckpt_dict_client = {
            'round': r + 1,
            'arch': args.arch,
            'state_dict': global_model_client.state_dict(),
            'best_acc1': best_acc1,
            'optimizer': optimizers_clients_models[i].state_dict(),
        }

        ckpt_dict_server = {
            'round': r + 1,
            'arch': args.arch,
            'state_dict': global_model_server.state_dict(),
            'best_acc1': best_acc1,
            'optimizer': optimizers_servers_models[i].state_dict(),
        }
        ...
        metric.save_checkpoint_client(
                ckpt_dict_client, is_best, args.model_dir)
        metric.save_checkpoint_server(
                ckpt_dict_server, is_best, args.model_dir)
        print('%d-th round' % r)
