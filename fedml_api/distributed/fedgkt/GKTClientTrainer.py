...
class GKTClientTrainer(object):
    def __init__(self, client_index, local_training_data, local_test_data, device,
                 client_model, args):
        ...
        logging.info("client device = " + str(self.device)+"init_Complete")
        ...
        if self.args.whether_training_on_client == 1:
            ...
            for epoch in range(self.args.fed_epochs):
                ...
                for batch_idx, (images, labels) in enumerate(self.local_training_data):
                    ...
                    logging.info('client {} - Update Epoch: {} [{}/{} ({:.0f}%)]'.format(
                        self.client_index, epoch, batch_idx * len(images), len(self.local_training_data.dataset),
                                                  100. * batch_idx / len(self.local_training_data)))
