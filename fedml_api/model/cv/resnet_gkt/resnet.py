...
class ResNetMainClient(nn.Module):
        ...
        if groups == 1:
            ...
            if 'wide_resnet' in arch:
                if arch in ['wide_resnetsl16_8']:
                    ...
                    # assert split_factor in [1, 2, 4] and (dataset ==
                    # 'imagenet' or dataset == 'ham10000' or dataset ==
                    # 'pill_base' or dataset == 'pill_large')

                    # The below is the same as max(widen_factor / (split_factor
                    # ** 0.5) + 0.4, 1.0)
                    if arch == 'wide_resnetsl50_2' and split_factor == 2:
                        self.inplanes = 64
                        width_per_group = 64
                        print(
                            'INFO:PyTorch: Dividing wide_resnet50_2, change base_width from {} '
                            'to {}.'.format(
        64 * 2, 64))
                    if arch == 'wide_resnet50_3' and split_factor == 2:
                        ...
                        print(
                            'INFO:PyTorch: Dividing wide_resnet50_3, change base_width from {} '
                            'to {}.'.format(
        64 * 3, 64 * 2))
                else:
                    raise NotImplementedError
    ...
    "or a 3-element tuple, got {}".format(replace_stride_with_dilation))
        ...
        if (dataset == 'ham10000' or dataset ==
                'pill_base' or dataset == 'pill_large'):
    ...
    padding=1,
     bias=False),
            ...
            # for training cifar, change the kernel_size=7 -> kernel_size=3
            # with stride=1
            self.layer0 = nn.Sequential(
    ...
    padding=1,
     bias=False),
            ...
            if 'wide_resnet' in arch:
                ...
                inplanes_origin = inplanes_origin * \
                    int(max(widen_factor / (split_factor ** 0.5) + 0.4, 1.0))

            # 32 -> 32 -> 16 -> 8
            strides = [1, 2, 2, 1]
            if output_stride == 2:
                print(
                    'INFO:PyTorch: Using output_stride {} on cifar10'.format(output_stride))
                strides = [1, 1, 2, 1]
            # print(block, inplanes_origin, layers[0], strides[0])
...
class ResNetProxyClients(nn.Module):
    ...
    "or a 3-element tuple, got {}".format(replace_stride_with_dilation))
        ...
        self.layer1 = self._make_layer(
            block, inplanes_origin, layers[0], stride=strides[0])
        ...
        if ('ham10000' in dataset or 'pill_base' in dataset or 'pill_large' in dataset):
            print('INFO:PyTorch: Using layer4 for ham and pill Training')
        ...
        if 'cifar' in dataset:
            if arch in ['wide_resnetsl16_8']:
                if dropout_p is not None:
                    ...
                    print(
                        'INFO:PyTorch: Using dropout with ratio {}'.format(dropout_p))
...
def wide_resnet16_8_gkt(pretrained=False, progress=True, **kwargs):
...
# Resnet 110 split 
def resnet110_gkt(pretrained=False, progress=True, **kwargs):
...
def wide_resnet50_2_gkt(pretrained=False, progress=True, **kwargs):
